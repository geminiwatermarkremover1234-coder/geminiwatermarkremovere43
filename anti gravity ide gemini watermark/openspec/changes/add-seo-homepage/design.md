...
`index.html` (1176 lines, "Lumina AI") already has the right skeleton: hero H1, best-practices H2, features, toolkit, 3-steps, how-to, FAQ. Copy is thin and keyword placement accidental. This change is copywriting + markup only — the sections stay, the words get rewritten and extended.
...
1. **Extend existing sections instead of adding a wall-of-text SEO block.** Google devalues detached "SEO paragraphs"; copy woven into real sections (hero, features, how-to, FAQ) reads naturally and converts.

2. **Keyword → section map** (each phrase used once, twice max in body copy):
